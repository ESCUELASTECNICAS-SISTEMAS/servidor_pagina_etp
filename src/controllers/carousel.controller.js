...
exports.list = async (req, res) => {
  try {
    ...
    const items = await db.CarouselSlide.findAll({ where, include: [{ model: db.Media, as: 'media', attributes: ['id','url','alt_text'] }], attributes: ['id','media_id','title','subtitle','description','order_index','active'], order: [['order_index','ASC']] });
...
exports.getById = async (req, res) => {
  try {
    const { id } = req.params;
    const item = await db.CarouselSlide.findByPk(id, { include: [{ model: db.Media, as: 'media', attributes: ['id','url','alt_text'] }], attributes: ['id','media_id','title','subtitle','description','order_index','active'] });
...
exports.create = async (req, res) => {
  try {
    const { media_id, title, subtitle, description, order_index } = req.body;
    if (!media_id) return res.status(400).json({ message: 'media_id required' });
    const item = await db.CarouselSlide.create({ media_id, title, subtitle, description, order_index: order_index || 0 });
...
exports.update = async (req, res) => {
  try {
    const { id } = req.params;
    const { media_id, title, subtitle, description, order_index, active } = req.body;
...
exports.updateText = async (req, res) => {
  try {
    ...
    if (typeof title === 'undefined' && typeof subtitle === 'undefined' && typeof description === 'undefined') {
      return res.status(400).json({ message: 'title, subtitle or description required' });
    }
